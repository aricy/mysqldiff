...
class SQLTransformer(object):
    ...
    def _get_column_position(destination_def, source_def, destination, source,
        ...
        #print "source_def",source_def
        #print "destination_def",destination_def
        if source_def[-1] == 'PRI' and (source_def[-3] != destination_def[-3] and source_def[-1] == destination_def[-1]):
    ...
    def _get_index_sql_clauses(rows, sql_mode=''):
        ...
        if rows != []:
            ...
            for key in rows:
                col_length = key[4]
                if key[7] != None:
                    col_length = col_length + "(" + key[7] + ")"
                if key[2] == 'PRIMARY':
                    q_key = quote_with_backticks(col_length, sql_mode)
                    ...
                    if unique_name is None:
                        ...
                        unique_key_cols.append(col_length)
